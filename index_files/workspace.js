...
r.addEventListener("play",function(){
    ...
    // Delay until animation is done (~2s, matches CSS Cue-In)
    setTimeout(function(){
        var smileBtn=document.createElement("button");
        smileBtn.textContent="made u smile. hehehehe. click here to explore more!!!";
        smileBtn.style.position="fixed";
        smileBtn.style.bottom="20px";
        smileBtn.style.left="50%";
        smileBtn.style.transform="translateX(-50%)";
        smileBtn.style.fontSize="14px";
        smileBtn.style.padding="10px 20px";
        smileBtn.style.background="#ff69b4";
        smileBtn.style.color="#000";
        smileBtn.style.border="none";
        smileBtn.style.borderRadius="12px";
        smileBtn.style.cursor="pointer";
    ...
    },7000); // 2s = same as CSS Cue-In animation
});


;var d=document.createElement("button");d.textContent="Trust me",d.addEventListener("click",function(e){d.parentNode.removeChild(d),i.appendChild(o),r.play()}),i.appendChild(d)}return a(n,[{key:"handleResize",value:function(){this.scene.size={width:Math.floor(window.innerWidth),height:Math.floor(window.innerHeight)}}}]),n}();function r(){window.controller=new o(document.body)}document.addEventListener("DOMContentLoaded",r);
